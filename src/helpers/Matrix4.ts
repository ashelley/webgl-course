export default class Matrix4 {
    ...
    raw:Float32Array //NOTE: LENGTH = 16
	...
	rotateY(radians:number){ 
        Matrix4.rotateY(this.raw,radians); 
        return this; 
    }

	rotateX(radians:number) {
        Matrix4.rotateX(this.raw,radians); 
        return this; 
    }

	rotateZ(radians:number) {
        Matrix4.rotateZ(this.raw,radians); 
        return this; 
    }
	...
	//NOTE: Bring is back to identity without changing the transform values.
	resetRotation(){	
		for(let i=0; i < this.raw.length; i++){
			if(i >= 12 && i <= 14) continue;
			this.raw[i] = (i % 5 == 0)? 1 : 0;  //NOTE: only positions 0,5,10,15 need to be 1 else 0.
		}
		return this;
	}

	//NOTE: Reset data back to identity.
	reset(){ 
		for(let i=0; i < this.raw.length; i++) {
            this.raw[i] = (i % 5 == 0)? 1 : 0; //NOTE: only positions 0,5,10,15 need to be 1 else 0.
        }
	...
	//NOTE: https://github.com/toji/gl-matrix/blob/master/src/gl-matrix/mat4.js
	//      make the rows into the columns
	static transpose(out, a){
		//NOTE: If we are transposing ourselves we can skip a few steps but have to cache some values
		if (out === a) {
            let a01 = a[1]
            let a02 = a[2]
            let a03 = a[3]
	...
	//NOTE: Calculates a 3x3 normal matrix (transpose inverse) from the 4x4 matrix
